...
listenToDockerEvents() {
    ...
    local LOGLINE
    expected_restarts=0
    docker events --filter "label=com.docker.compose.project=${COMPOSE_PROJECT_NAME}" --filter 'event=restart' | while read -r LOGLINE; do
        echo "$LOGLINE"
        # may be more elaborate checks here.
        [[ $LOGLINE == *"container restart "*"com.docker.compose.service=ignore"* && $LOGLINE == *"com.docker.compose.project=$COMPOSE_PROJECT_NAME"* ]] && echo "ERR: No restarts expected on ignore-* containers!" 1>&2 && pkill -9 docker && exit 1
